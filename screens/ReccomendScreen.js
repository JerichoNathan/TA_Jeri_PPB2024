...
import ReccomendCard  from '../components/ReccomendCard';
...
export default function TopLN({ route, navigation }) {
    ...
    return (
        <ImageBackground
            source={{ uri: 'https://arknights.wiki.gg/images/8/8a/Home_Screen_background-Mystic_Sea.png' }}
            style={{ flex: 1 }}
        >
            ...
            <View style={{ flex: 1, }}>
                <View
                    style={{
                        ...
                        backgroundColor: 'rgba(97, 211, 237, 0.8)'
                    }}
                >
                ...
                >
                    {dataLN.map((item, index) => (
                        <TouchableOpacity onPress={() => navigation.navigate('Detail', {data: item,})} >
                            <ReccomendCard key={item.mal_id ? item.mal_id.toString() : "defaultKey"} dataLN={item} noRank={25* (count-1) + index + 1}  />
                        </TouchableOpacity>
